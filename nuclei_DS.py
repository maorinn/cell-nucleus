...
def process(data_folder, model_name, format):
    ...
    for index, temp_name in enumerate(name_list):
        ...
        print("os.path.join(temp_path, temp_name+format",
              os.path.join(temp_path, temp_name+format))
        temp_image = cv2.imread(os.path.join(
            temp_path, temp_name+format))  # 读取样本图片
        if temp_image is None:
            raise AssertionError(temp_path, ' not found')
        batch_group, shape = preprocess(
            temp_image, patch_size, stride, temp_path)
        temp_image = resize(
            temp_image,
            (128, 128),
            mode='constant',
        ...
        X_test[0] = temp_image
        skimage.io.imshow(temp_image.astype(np.uint8))
        skimage.io.show()
        # mask_list = sess_interference(model, batch_group)
        mask_list = model.model_saver.predict(X_test, verbose=1)
        ...
        _image = cv2.imread(os.path.join(
            temp_path, temp_name+format))  # 读取样本图片
        center_edge_mask, gray_map = center_edge(c_mask, _image)
